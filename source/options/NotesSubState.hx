...
class NotesSubState extends MusicBeatSubstate
{
	...
	var tipTxt:FlxText;
	var sustainLength(default, set):Float = 350;
	var susAngle(default, set):Float = 0;
	...
	public function new()
	{
		...
		var sustainStench:FlxUISlider = new FlxUISlider(this, 'sustainLength', 500, FlxG.height * 0.8, 0, 533.3, 100, 6, 10, 0xFF000000, 0xFF0084FF);
		add(sustainStench);

		var sustainAngle:FlxUISlider = new FlxUISlider(this, 'susAngle', sustainStench.x, sustainStench.y + 37.232352, 0, 360, 100, 6, 10, 0xFF000000,
			0xFF0084FF);
		add(sustainAngle);
	...
	public function spawnNotes()
	{
		...
		bigNote.setGraphicSize(120);
		bigNote.sustainLength = sustainLength;
		...
		sustain = new Sustain(bigNote);
		bigNote.sustain = sustain;
		sustain.angle = susAngle;
		insert(members.indexOf(bigNote), sustain);
	...
	function set_sustainLength(value:Float):Float
	{
		...
		if (bigNote != null)
			bigNote.sustainLength = value;

		return sustainLength = value;
	}

	function set_susAngle(value:Float):Float
	{
		susAngle = value;
