...
class DialogueBox extends FlxSpriteGroup
{
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		if (PlayState.SONG.song.toLowerCase() == 'senpai erect')
		{
			var rimDad = new DropShadowShader();
			rimDad.setAdjustColor(-66, -10, 24, -23);
			rimDad.color = 0xFF52351d;
			rimDad.antialiasAmt = 0;
			rimDad.attachedSprite = portraitLeft;
			rimDad.distance = 5;

			rimDad.angle = 90;
			portraitLeft.shader = rimDad;

			rimDad.altMaskImage = Paths.image('weeb/senpaiPortraitmask').bitmap;
			rimDad.maskThreshold = 1;
			rimDad.useAltMask = true;

			portraitLeft.animation.onFrameChange.add(function(name:String, frame:Int, index:Int)
			{
				if (portraitLeft != null)
					rimDad.updateFrameInfo(portraitLeft.frame);
			});
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (PlayState.SONG.song.toLowerCase() == 'roses' || PlayState.SONG.song.toLowerCase() == 'roses erect')
			portraitLeft.visible = false;
	...
	function startDialogue():Void
	{
		...
		switch (curCharacter)
		{
			case 'dad':
				...
				{
					if (PlayState.SONG.song.toLowerCase() == 'senpai' || PlayState.SONG.song.toLowerCase() == 'senpai erect')
